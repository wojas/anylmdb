...
The wrapper code is Windows-clean (MSVC and MinGW-w64): the sniffer uses
`CreateFileW` with UTF-8 paths like the engines do, and no POSIX-only APIs
remain outside the engines. `scripts/msvc-build.ps1` builds and tests with
`cl` (also used by CI); consumers like py-lmdb would simply compile the
eight `anylmdb_*.c` files with their extension.

**Upstream caveat:** LMDB 1.0.0 itself does not compile for Windows —
`mdb.c` misuses the `LARGE_INTEGER` union in `mdb_env_copyfd0` and
`mdb_env_incr_loadfd` (hard errors under both MSVC and MinGW; still present
on upstream's `mdb.RE/1.0` branch as of 2026-07-10). This is the one place
anylmdb carries a patch: Windows builds compile `mdb_lmdb10_win.c.h`, which
is the pristine source plus the minimal checked-in `mdb_lmdb10_win.patch`
(six hunks, compile fixes only), regenerated by `scripts/gen-win10.sh` at
vendoring time. POSIX builds always compile the byte-identical upstream
source. The patch should be dropped as soon as upstream ships a fixed
1.0.x release.
